/* elevator_constrained_fire_4.pml
   Constrained 4-floor model starting in FIRE mode.
   Monitor includes fire_goal_reached progress label when ground floor reached and doors open.
*/
...
init {
  byte i;
  atomic {
    /* initialize requests: set all normal requests to false (FIRE only) */
    i = 0;
    do :: (i < N_FLOORS) -> rq[i] = false; i = i + 1; :: else -> break; od;
    floor = 3; target = 0; mode = FIRE; door = CLOSED; moving = false;
...
/* Monitor: detect service events */
proctype Monitor()
{
  do
  :: (mode == FIRE && floor == 0 && !moving && door == OPEN) ->
    fire_goal_reached: printf("LABEL fire_goal_reached fired\n");
