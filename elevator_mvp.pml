...
bool vip_req = false;     /* VIP request active1w */
...
proctype Elevator()
{
  do
  ::
    /* CRITICAL: assert the safety property at the TOP of every loop iteration */
    ...
    /* CRITICAL: assert the safety property at the BOTTOM of every loop iteration */
    assert(!(moving && door == OPEN));
  od
}
