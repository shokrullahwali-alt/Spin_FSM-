/* elevator_constrained_4.pml
   Constrained 4-floor model: deterministic environment (all normal requests present),
   uses Controller + Elevator processes to exercise realistic behavior but without
   nondeterministic events like FIRE/VIP toggles.
*/
...
/* Progress labels: per-floor service points (when elevator reaches a floor and opens)
   We mark service when Elevator is at target and door opens; to ensure labels appear,
   add small monitor that checks and labels when conditions hold. */
